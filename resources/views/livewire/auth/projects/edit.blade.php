                        ...
                        <div class="form-section">
                            <div class="form-group mb-3">
                                <div
                                    ...
                                    <input @if($this->tumbnail) disabled @endif type="file" class="form-control @error('tumbnail') is-invalid @enderror" id="tumbnail" wire:model.live="tumbnail">
                                    ...
                                    @if($this->tumbnail)
                                        ...
                                        <div class="project-upload-preview-container">
                                            <div class="project-upload-preview-box">
                                                <i wire:click="removeTumbnailImage()" class="fa-solid fa-trash"></i>
                                                @if(is_string($this->tumbnail))
                                                <img src="{{asset('storage/images/frontend/projects/'.$this->project->friendly_route.'/'.$this->tumbnail) }}"/>
                                                @else
                                                    <img src="{{ $this->tumbnail->temporaryUrl() }}"/>
                                                @endif
                                            </div>
                                        </div>
                                    @endif
                        ...
                        <div class="form-section">
                            <div wire:ignore>
                                <div class="form-group mb-3" wire:ignore>
                                    ...
                                    <textarea id="small_description" wire:model.live="small_description">{{$this->small_description}}</textarea>
                        ...
                        <div class="form-section">
                            <div wire:ignore>
                                <div class="form-group mb-3" wire:ignore>
                                    ...
                                    <textarea id="description" wire:model="description">{{$this->description}}</textarea>
                                    @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="form-section">
                            <div class="form-group mb-3">
                                <div
                                    ...
                                    @if(count($this->existing_project_images))

                                        <br/>
                                        <h5 class="form-section-title">Bestaande afbeeldingen:</h5>
                                        <div class="project-upload-preview-container">
                                            @foreach($this->existing_project_images as $key => $image)
                                                <div  class="project-upload-preview-box">
                                                    <i wire:click.prevent="removeImages('{{$image->id}}','{{$loop->index}}')" class="fa-solid fa-trash"></i>
                                                    <img src="{{asset('storage/images/frontend/projects/'.$this->project->friendly_route.'/'.$image->image) }}"/>
                                                </div>
                                            @endforeach
                                        </div>
                                            @endif
                                                @if(count($this->project_images))
                                                    <br/>
                                                    <h5 class="form-section-title">Nieuw geuploade foto(s):</h5>
                                                    <div class="project-upload-preview-container">

                                                        @foreach($project_images as $key => $image)
                                                            <div  class="project-upload-preview-box">
                                                                @if(str_contains($image, '.tmp'))
                                                                    <i wire:click.prevent="removeTempImages({{$loop->index}})" class="fa-solid fa-trash"></i>
                            ...
                            <div class="d-grid gap-2">
                                <button wire:click.prevent="updateProject()" :disabled="buttonDisabled" class="btn btn-success btn-block">Opslaan</button>
